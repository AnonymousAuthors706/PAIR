 ...
 *
 * Violation Example #2: Buffer overflow on function pointer violation
 *      Based on: https://github.com/RIT-CHAOS-SEC/ACFA/blob/main/demo_prv/main.c
 *
 * Expected behaviour:
 *      P3OUT : 0x50 (then 0x05 if non-attack)
 *      P1OUT : 0x01
 *
 * Approx. sim time: 10ms
 */
...
// TARGET ADDRESS OF ATTACK: location of "access granted" -- 0xd1ea
#define TARGET_UPPER 0xd1
#define TARGET_LOWER 0xea

// Password defines
#define size 4
#define user_size 5
#define attack_size 7
#define cr '\r'
...
// waitForPassword
char user_input[attack_size] = {0x01, 0x02, 0x03, 0x04, TARGET_LOWER, TARGET_UPPER, '\r'};
...
struct pwd_info { /* use struct to ensure correct memory layout in stack */
        char entry[4];
        void (*fptr)(void);
};
...
char waitForPassword()
{
        ...
        struct pwd_info info;
        info.fptr = dummy;

        read_data(info.entry);

        char total = 0;
        unsigned int i;
        for (i = 0; i < 4; i++) {
                total |= (pass[i] ^ info.entry[i]);
        }

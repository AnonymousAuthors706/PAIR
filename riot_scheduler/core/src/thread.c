...
#pragma vector=VIOLATION_VECTOR
__attribute__ ((naked)) __interrupt void acfi_violation_handler(void)
{
    // hw-generated interrupt when violation occurs
    // proceed to zombify the current (violating) thread
    thread_zombify();
}

...
/// openmsp430 versions of these functions
static inline void atomic_store_u16(uint16_t * dest, uint16_t src) {
    dint();
    *dest = src;
    eint();
}

static inline uint16_t atomic_load_u16(uint16_t * src) {
    dint();
    uint16_t val = *src; 
    eint();
    return val;
...
static int _lock(rmutex_t *rmutex, int trylock)
{
    ...
    if (mutex_trylock(&rmutex->mutex) == 0) {
        ...
        owner = (kernel_pid_t) atomic_load_u16((uint16_t *)(&rmutex->owner));
    ...
    atomic_store_u16((uint16_t *)&rmutex->owner, thread_getpid());
...
void rmutex_unlock(rmutex_t *rmutex)
{
    ...
    if (rmutex->refcount == 0) {
        ...
        atomic_store_u16((uint16_t *)(&rmutex->owner), KERNEL_PID_UNDEF);
